function FIGURE_dangerous(data, emulator_data)
...
for k = 1:length(densities)
    ...
    for i = 1:2
        xlabel( pax{k,2,i,1}, param_names{i}, 'Fontsize', 20);
        ylabel( pax{k,i,1,1}, param_names{i}, 'Fontsize', 20);
    end
    
%     % Append parameter names to the bottom of each column
%     for m = 1:3
%         text( pax{k,3,m,1}, (pax{k,3,m,1}.XLim(1) + pax{k,3,m,1}.XLim(2))/2, pax{k,3,m,1}.YLim(1) - (pax{k,3,m,1}.YLim(2) - pax{k,3,m,1}.YLim(1)) * 0.25, param_names{m}, 'Fontsize', 20, 'HorizontalAlignment', 'center', 'VerticalAlignment', 'middle');
%     end
%     % Append parameter names to the left of each row
%     for m = 1:3
%         text( pax{k,m,1,1}, pax{k,m,1,1}.XLim(1) - (pax{k,m,1,1}.XLim(2) - pax{k,m,1,1}.XLim(1)) * 0.25, (pax{k,m,1,1}.YLim(1) + pax{k,m,1,1}.YLim(2))/2, param_names{m}, 'Fontsize', 20, 'HorizontalAlignment', 'right', 'VerticalAlignment', 'middle');
%     end
%     
%     % Turn off all X tick labels in middle row
%     for n = 2
%         for m = 1:3
%             pax{k,n,m,1}.XTick = [];
%         end
%     end
%     % Turn off all Y tick labels in middle column
%     for n = 2
%         for m = 1:3
%             pax{k,m,n,1}.YTick = [];
%         end
%     end
%     % Flip X tick labels in top row
%     for n = 1
%         for m = 1:3
%             pax{k,n,m,1}.XAxisLocation = 'top';
%         end
%     end
%     % Flip Y tick labels in right column
%     for n = 3
%         for m = 1:3
%             pax{k,m,n,1}.YAxisLocation = 'right';
%         end
%     end
    ...
    axis(pax{k,1,2,1}, [0 1 0 1]);
    text(pax{k,1,2,1}, 0.5, 0.5, ['$\phi = $',num2str(densities(k))], 'Interpreter','LaTeX','Fontsize', 35, 'HorizontalAlignment', 'center', 'VerticalAlignment','middle');
