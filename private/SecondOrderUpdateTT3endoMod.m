function [I_ion, S, Sinf, invtau, I_Na, I_CaL, I_Kr, I_Ks, I_to, I_K1, I_NaK, I_NaCa] = SecondOrderUpdateTT3endoMod(V, S, S_old, Sinf_old, invtau_old, dt, I_stim, I_stim_old, params)
...
% Define the base extracellular concentration of K+ (in the case of zero
% hyperkalemia)
K_o_base = 5.4;
...
g_CaL = g_CaL * params(6);                   % Reduction in I_CaL due to pH change
k_NaCa = k_NaCa * params(7);                 % Reduction in I_NaCa due to pH change (no reduction currently)
...
I_KATP =  0.000195 / nichols_area / ( 1 + ( ATP_i / k_ATP ) ^ h_ATP ) * ( K_o / K_o_base )^power_ATP_K .* dV_K;
