function [I_ion, S, Sinf, invtau] = processReaction(V, S, S_old, Sinf_old, invtau_old, dt, I_stim, I_stim_old, cell_models, model_assignments, mesh, second_order, extra_params)
    ...
    if sum(batch) > 0
        ...
        %try 
            ...
            if second_order
                ...
                % Otherwise, do.
                if isempty(Sinf_old)  
                    [I_ion(batch), S_batch, Sinf_batch, invtau_batch] = feval(['SecondOrderUpdate',this_model], V(batch), S(batch,:), S_old(batch,:), [], [], dt, I_stim(batch), I_stim_old(batch,:), extra_params);
                else
                    [I_ion(batch), S_batch, Sinf_batch, invtau_batch] = feval(['SecondOrderUpdate',this_model], V(batch), S(batch,:), S_old(batch,:), Sinf_old(batch,:), invtau_old(batch,:), dt, I_stim(batch), I_stim_old(batch,:), extra_params);
                end
